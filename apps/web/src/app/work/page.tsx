...
export const metadata = createMetadata({ title: "Work", description: "Concept websites, software and mobile products by Illustriober Creatives.", path: "/work" });
    ...
    <main className="min-h-screen bg-[#F4EFE5] px-5 pb-20 pt-40 text-[#171717] md:px-8 md:pt-48">
      <div className="mx-auto max-w-7xl">
        <ScrollReveal blur y={22} scale={0.99}>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#1F4D3D]">Concept projects</p>
          <div className="mt-5">
            <h1 className="max-w-4xl font-display text-5xl leading-[0.9] tracking-[-0.05em] md:text-7xl">Websites, systems and apps in practice.</h1>
            <p className="mt-6 max-w-xl text-base leading-7 text-[#5F5A50]">These are concept builds that show the kinds of business problems we can solve. They are examples of our approach, not client case studies.</p>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal blur className="mt-12" scale={0.985} y={24}>
          <div className="flex flex-col gap-5 rounded-[2rem] bg-[#F39314] px-6 py-7 transition-[transform,box-shadow] duration-[700ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-safe:hover:-translate-y-1 motion-safe:hover:shadow-[0_24px_70px_rgba(243,147,20,0.22)] motion-reduce:transition-none sm:px-8 md:flex-row md:items-center md:justify-between md:py-8">
            <h2 className="whitespace-nowrap font-display text-[clamp(1.7rem,4vw,3.5rem)] leading-none">
              Have a product you want us to build?
            </h2>
