  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.card} aria-live="polite">
        ...
        {step === "schedule" && stage(<>When shall we<br /><em>make it happen?</em></>, "Your calendar, your call", <div className={styles.formGrid}><label>What day feels good?<input type="date" value={preferredDate} onChange={(event) => setPreferredDate(event.target.value)} /></label><label>Best time<select value={timeOfDay} onChange={(event) => setTimeOfDay(event.target.value)}><option value="">Select a time</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option><option value="night">Night</option></select></label></div>, Boolean(preferredDate && timeOfDay), nextAfterSchedule, "Save the date")}
