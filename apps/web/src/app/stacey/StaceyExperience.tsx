...
const noCaptions = [
  "Tap the banner if you really mean it.",
  "Are you sure? I’d really love to take you out.",
  "One hundred percent sure?",
  "No pressure at all — I just had to ask.",
  "Okay, I hear you. One more tap and I’ll let you go gracefully.",
];
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.card} aria-live="polite">
        {step === "reveal" && <div className={styles.reveal}><p className={styles.eyebrow}>For Stacey, with a little courage</p><div className={styles.photoFrame}><div className={styles.photoClip}><Image src={photos[0]} alt="Stacey smiling" width={600} height={492} priority /></div></div><h1>A little something<br />made just for you.</h1><button className={styles.primaryButton} onClick={() => setStep("question")}>Open it <span>→</span></button><p className={styles.tiny}>P.S. I hope this makes you smile.</p></div>}
        {step === "question" && <div className={styles.question}><button className={styles.backButton} onClick={() => setStep("reveal")} aria-label="Go back to the opening">← Back</button><p className={styles.eyebrow}>A small confession</p><h1>You&apos;ve been on<br /><em>my mind.</em></h1><p className={styles.note}>I think you&apos;re genuinely gorgeous, and I&apos;ve been a little too shy to say it out loud. So I made this instead: would you let me take you out and make a lovely memory together?</p><div className={styles.answerRow}><button className={styles.primaryButton} onClick={() => setStep("activity")}>Yes, I&apos;d love to! <span>♡</span></button><div className={styles.noArea}><div className={styles.mascotRunner} style={{ transform: noPositions[declineAttempts] }}><Image src={`/stacey/courage-${declineAttempts}.png`} alt="An illustrated version of the person asking, holding a banner" width={260} height={320} /><button className={styles.mascotBanner} onClick={tryDecline}>{noPrompts[declineAttempts]}</button></div></div></div><p className={styles.dodgeHint}>{noCaptions[declineAttempts]}</p></div>}
