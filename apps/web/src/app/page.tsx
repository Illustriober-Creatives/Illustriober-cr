  ...
  return (
    <div className="overflow-hidden bg-[#F4EFE5] text-[#171717]">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-20 md:px-8 md:py-28 lg:grid-cols-[0.78fr_1.22fr]">
        ...
        <div className="grid gap-5 sm:grid-cols-3">{[["01", "Make it clear", "Strategy, structure, and an interface people understand."], ["02", "Make it useful", "A focused build around the work that matters most."], ["03", "Make it last", "A maintainable product your team can keep improving."]].map(([number, title, text], index) => <ScrollReveal delay={index * 0.06} y={22} key={number}><article className="border-t-2 border-[#171717] pt-4"><p className="text-xs font-bold text-[#F39314]">{number}</p><h3 className="mt-5 text-lg font-bold">{title}</h3><p className="mt-3 text-sm leading-6 text-[#5F5A50]">{text}</p></article></ScrollReveal>)}</div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 md:px-8 md:pb-28">
        <div className="grid overflow-hidden rounded-[2rem] border border-black/10 bg-[#FFFDF8] lg:grid-cols-3">
          {[
            ...
            return <ScrollReveal className="h-full" delay={index * 0.07} y={18} key={title as string}><article className={`h-full p-7 md:p-9 ${index < 2 ? "border-b border-black/10 lg:border-b-0 lg:border-r" : ""}`}><CapabilityIcon aria-hidden="true" className="h-7 w-7 text-[#1F4D3D]" /><h3 className="mt-10 font-display text-3xl leading-none">{title as string}</h3><p className="mt-4 text-sm leading-6 text-[#5F5A50]">{copy as string}</p></article></ScrollReveal>;
          })}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 md:px-8 md:pb-28">
        <div className="grid gap-10 border-t border-black/15 pt-10 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <ol className="grid gap-7 sm:grid-cols-2">
            {[['01', 'Frame', 'Define the problem, audience, constraints, and a measurable finish line.'], ['02', 'Prototype', 'Make the important journey tangible before committing to the full build.'], ['03', 'Build', 'Deliver in reviewable slices with decisions, risks, and progress kept visible.'], ['04', 'Launch & learn', 'Verify the real experience, document the system, and plan what evidence says comes next.']].map(([number, title, copy], index) => <ScrollRevealListItem className="border-t-2 border-[#171717] pt-4" delay={index * 0.05} y={20} key={number}><p className="text-xs font-bold text-[#F39314]">{number}</p><h3 className="mt-4 text-lg font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-[#5F5A50]">{copy}</p></ScrollRevealListItem>)}
          </ol>
        </div>
      </section>
