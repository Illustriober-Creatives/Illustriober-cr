  ...
  return (
    <div className="overflow-hidden bg-[#F4EFE5] text-[#171717]">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-20 md:px-8 md:py-28 lg:grid-cols-[0.78fr_1.22fr]">
        <div><h2 className="font-display text-5xl leading-[0.92] tracking-[-0.045em] md:text-6xl">From a rough idea to something people can use.</h2></div>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 md:px-8 md:pb-28" id="work">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-5"><div><h2 className="font-display text-5xl leading-none tracking-[-0.045em] md:text-6xl">What a better system can look like.</h2></div><Link className="inline-flex items-center gap-2 text-sm font-bold underline decoration-[#F39314] decoration-2 underline-offset-4" href="/work">View all demo projects <ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link></div>
        <ProjectGallery limit={4} />
      </section>

      <section className="mx-auto max-w-7xl px-5 pb-20 md:px-8 md:pb-28">
        <div className="grid gap-10 border-t border-black/15 pt-10 lg:grid-cols-[0.7fr_1.3fr]">
          <div><h2 className="font-display text-4xl leading-none md:text-5xl">Know what is happening and why.</h2></div>
      ...
      <section className="bg-[#1F4D3D] px-5 py-20 text-[#F4EFE5] md:px-8 md:py-28"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_0.82fr] lg:items-start"><h2 className="max-w-3xl text-balance font-display text-5xl leading-[1.02] tracking-[-0.045em] md:text-7xl">A practical partner for the work after the idea.</h2><ul className="space-y-4 text-base text-[#F4EFE5]/85 lg:pt-2">{["A clear scope before the build starts", "Visible progress instead of mystery", "A product your team can own after launch"].map((item) => <li className="flex gap-3" key={item}><Check className="mt-0.5 h-5 w-5 shrink-0 text-[#F7AD45]" aria-hidden="true" />{item}</li>)}</ul></div></section>

      <section className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-28"><div className="rounded-[2rem] bg-[#F39314] px-7 py-14 text-center md:px-12 md:py-20"><h2 className="mx-auto max-w-3xl text-balance font-display text-5xl leading-[1.02] tracking-[-0.045em] md:text-7xl">Got a thing that needs building?</h2><p className="mx-auto mt-6 max-w-xl text-lg leading-7 text-[#171717]/75">Tell us what needs to work better. We’ll help map the next sensible move.</p><Link className="mt-8 inline-flex min-h-12 items-center gap-2 rounded-full bg-[#171717] px-7 text-sm font-bold text-white transition-transform hover:-translate-y-0.5" href="/enquiry">Start a conversation <ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link></div></section>
    </div>
  );
}
