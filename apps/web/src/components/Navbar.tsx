      ...
      <div className="border border-black/10 bg-[#FFFDF8]/95 px-4 py-3 shadow-[0_8px_24px_rgba(23,23,23,0.08)] backdrop-blur md:rounded-full md:px-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="hidden items-center gap-4 md:flex">
            {user ? (
              <Link className="rounded-full bg-[#171717] px-4 py-2.5 text-sm font-bold text-[#F4EFE5] transition-transform hover:-translate-y-0.5" href="/dashboard">Dashboard</Link>
            ) : (
              <>
                <Link className="text-sm font-semibold text-[#5F5A50] hover:text-[#171717]" href="/login">Sign in</Link>
        ...
        {isMenuOpen && (
          <div className="mt-4 grid gap-1 border-t border-black/10 pt-3 md:hidden">
            {navLinks.map((link) => (
              <Link className="rounded-lg px-3 py-3 text-sm font-semibold text-[#5F5A50] hover:bg-[#F4EFE5] hover:text-[#171717]" href={link.href} key={link.href} onClick={closeMenu}>
                {link.label}
              </Link>
            ))}
            {user ? (
              <Link className="mt-2 rounded-full bg-[#171717] px-4 py-3 text-center text-sm font-bold text-[#F4EFE5]" href="/dashboard" onClick={closeMenu}>
                Dashboard
              </Link>
            ) : (
              <>
                <Link className="rounded-lg px-3 py-3 text-sm font-semibold text-[#5F5A50] hover:bg-[#F4EFE5]" href="/login" onClick={closeMenu}>
                  Sign in
                </Link>
                <Link className="mt-2 rounded-full bg-[#171717] px-4 py-3 text-center text-sm font-bold text-[#F4EFE5]" href="/enquiry" onClick={closeMenu}>
                  Start a project
                </Link>
              </>
            )}
