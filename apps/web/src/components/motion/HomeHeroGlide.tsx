...
export function HomeHeroGlide({ copy, media }: HomeHeroGlideProps) {
  ...
  return (
    <section ref={sectionRef} className="relative mx-auto max-w-[90rem] px-5 pb-16 pt-12 md:px-8 md:pb-20 md:pt-20 lg:pb-14 lg:pt-[7.25rem]">
      <div className="grid gap-12 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,0.7fr)] lg:items-start lg:gap-16 xl:gap-20">
        <motion.div
          className="min-w-0 overflow-x-hidden lg:relative lg:left-[10px]"
          style={shouldReduceMotion ? undefined : { y: copyY, opacity: copyOpacity }}
        >
          {copy}
        </motion.div>
        <motion.div
          className="relative mx-auto w-full max-w-2xl rounded-[1.8rem] bg-white p-3 shadow-[18px_22px_0_#1F4D3D] lg:-top-[10px] lg:left-[-10px] lg:mt-4 lg:max-w-none"
